...
$Port = if ($env:WEEK11_PGPORT) { $env:WEEK11_PGPORT } else { "55433" }
$RuntimeRoot = Join-Path $env:TEMP "hhk_week11_postgresql_runtime_utf8"
$DataDir = Join-Path $RuntimeRoot "data"
$LogDir = Join-Path $Root "logs"
$ServerOutLog = Join-Path $LogDir "postgresql_temp_server_stdout_week11.log"
$ServerErrLog = Join-Path $LogDir "postgresql_temp_server_stderr_week11.log"
$RunLog = Join-Path $LogDir "postgresql_temp_run_week11.log"
...
try {
    ...
    Invoke-LoggedExternal -Exe $Psql -Arguments @("-h", "127.0.0.1", "-p", $Port, "-U", "postgres", "-d", "postgres", "-v", "ON_ERROR_STOP=1", "-f", "database\schema_postgresql_week11.sql") -StepName "schema" -LogPath $RunLog
    Invoke-LoggedExternal -Exe $Psql -Arguments @("-h", "127.0.0.1", "-p", $Port, "-U", "postgres", "-d", "postgres", "-v", "ON_ERROR_STOP=1", "-f", "database\import_week11_tables.sql") -StepName "import" -LogPath $RunLog
    Invoke-LoggedExternal -Exe $Psql -Arguments @("-h", "127.0.0.1", "-p", $Port, "-U", "postgres", "-d", "postgres", "-v", "ON_ERROR_STOP=1", "-f", "database\postgres_week11_queries.sql") -StepName "queries" -LogPath $RunLog
