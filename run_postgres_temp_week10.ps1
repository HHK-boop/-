...
$Port = if ($env:WEEK10_PGPORT) { $env:WEEK10_PGPORT } else { "55432" }
$RuntimeRoot = Join-Path $env:TEMP "hhk_week10_postgresql_runtime"
$DataDir = Join-Path $RuntimeRoot "data"
$LogDir = Join-Path $Root "logs"
$ServerLog = Join-Path $LogDir "postgresql_temp_server_week10.log"
$ServerOutLog = Join-Path $LogDir "postgresql_temp_server_stdout_week10.log"
$ServerErrLog = Join-Path $LogDir "postgresql_temp_server_stderr_week10.log"
$RunLog = Join-Path $LogDir "postgresql_temp_run_week10.log"
...
try {
    ...
    if (-not (Test-Path -LiteralPath (Join-Path $DataDir "PG_VERSION"))) {
        Invoke-LoggedExternal -Exe $InitDb -Arguments @("-D", $DataDir, "-U", "postgres", "-A", "trust", "-E", "UTF8") -StepName "initdb" -LogPath $RunLog
    }
    ...
    Invoke-LoggedExternal -Exe $Psql -Arguments @("-h", "127.0.0.1", "-p", $Port, "-U", "postgres", "-d", "postgres", "-v", "ON_ERROR_STOP=1", "-f", "database\schema_postgresql_week10.sql") -StepName "schema" -LogPath $RunLog
    Invoke-LoggedExternal -Exe $Psql -Arguments @("-h", "127.0.0.1", "-p", $Port, "-U", "postgres", "-d", "postgres", "-v", "ON_ERROR_STOP=1", "-f", "database\import_week10_tables.sql") -StepName "import" -LogPath $RunLog
    Invoke-LoggedExternal -Exe $Psql -Arguments @("-h", "127.0.0.1", "-p", $Port, "-U", "postgres", "-d", "postgres", "-v", "ON_ERROR_STOP=1", "-f", "database\postgres_week10_queries.sql") -StepName "queries" -LogPath $RunLog
